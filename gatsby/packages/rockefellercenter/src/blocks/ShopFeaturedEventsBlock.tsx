...
const SHOP_FEATURED_EVENTS_QUERY = graphql`
  query ShopFeaturedEvents {
    allSanityEvent(filter: {category: {eq: "Shopping"}}) {
      edges {
        node {
          startEndDateTime {
...
const ShopFeaturedEventsBlock = (
  props: ComponentPropsWithoutRef<typeof AnchorSection>,
): JSX.Element | null => {
  const {allSanityEvent} = useStaticQuery<GatsbyTypes.ShopFeaturedEventsQuery>(
    SHOP_FEATURED_EVENTS_QUERY,
  );
  const data = useMemo(() => {
...
export default ShopFeaturedEventsBlock;
