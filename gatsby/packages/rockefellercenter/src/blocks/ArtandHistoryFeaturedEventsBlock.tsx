...
const ART_AND_HISTORY_FEATURED_EVENTS_QUERY = graphql`
  query ArtandHistoryFeaturedEvents {
    featuredEvents: sanityArtLp(_id: {eq: "artLP"}, _type: {eq: "artLP"}) {
      featuredEvents {
...
const ArtandHistoryFeaturedEventsBlock = (
  props: ComponentPropsWithoutRef<typeof AnchorSection>,
): JSX.Element | null => {
  const {
    featuredEvents,
  } = useStaticQuery<GatsbyTypes.ArtandHistoryFeaturedEventsQuery>(
    ART_AND_HISTORY_FEATURED_EVENTS_QUERY,
  );
  ...
  return featuredData.length ? (
    <AnchorSection {...props}>
      <Box sx={{py: [0, 0, 6], px: 0, mx: 'auto', maxWidth: 1320}}>
...
export default ArtandHistoryFeaturedEventsBlock;
