...
const BAR_SIXTY_FIVE_FEATURED_EVENTS_QUERY = graphql`
  query BarSixtyFiveFeaturedEvents {
    featuredEvents: sanityAttractionBar(
      _id: {eq: "bar65"}
...
const BarSixtyFiveFeaturedEventsBlock = (
  props: ComponentPropsWithoutRef<typeof AnchorSection>,
): JSX.Element | null => {
  const {
    featuredEvents,
  } = useStaticQuery<GatsbyTypes.BarSixtyFiveFeaturedEventsQuery>(
    BAR_SIXTY_FIVE_FEATURED_EVENTS_QUERY,
  );
  ...
  return featuredData.length ? (
    <AnchorSection {...props}>
      <Box sx={{py: [4, 6], pl: [4, 0], mx: 'auto', maxWidth: 1320}}>
...
export default BarSixtyFiveFeaturedEventsBlock;
