...
const RAINBOW_ROOM_FEATURED_EVENTS_QUERY = graphql`
  query RainbowRoomFeaturedEvents {
    featuredEvents: sanityAttractionRainbow(
      _id: {eq: "rainbowRoom"}
...
const RainbowRoomFeaturedEventsBlock = (
  props: ComponentPropsWithoutRef<typeof AnchorSection>,
): JSX.Element | null => {
  const {
    featuredEvents,
  } = useStaticQuery<GatsbyTypes.RainbowRoomFeaturedEventsQuery>(
    RAINBOW_ROOM_FEATURED_EVENTS_QUERY,
  );
  ...
  return featuredData.length ? (
    <AnchorSection {...props}>
      <Box sx={{py: 3, pl: [4, 0], mx: 'auto', maxWidth: 1320}}>
...
export default RainbowRoomFeaturedEventsBlock;
