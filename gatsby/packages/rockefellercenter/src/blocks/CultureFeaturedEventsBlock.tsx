...
const CULTURE_FEATURED_EVENTS_QUERY = graphql`
  query CultureFeaturedEvents {
    sanityCultureLp(_id: {eq: "cultureLP"}, _type: {eq: "cultureLP"}) {
      featuredEvents {
...
const CultureFeaturedEventsBlock = (
  props: ComponentPropsWithoutRef<typeof Section>,
): JSX.Element | null => {
  const {
    sanityCultureLp,
  } = useStaticQuery<GatsbyTypes.CultureFeaturedEventsQuery>(
    CULTURE_FEATURED_EVENTS_QUERY,
  );

  const events = sanityCultureLp?.featuredEvents;
...
export default CultureFeaturedEventsBlock;
