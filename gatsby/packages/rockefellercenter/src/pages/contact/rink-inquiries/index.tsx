...
export const query = graphql`
  query RinkInquiriesForm {
    meta: formJson(id: {eq: "rink-inquiry-form"}) {
      meta {
        title
        description
      }
    }
    formJson(id: {eq: "rink-inquiry-form"}) {
      id
      integrationId
      name
      slug
...
interface RinkInquiryFormValues {
  fullname: string;
  email: string;
  phone: string;
  message?: string;
...
const RinkInquiryForm = ({
  data: {formJson},
}: {
  data: GatsbyTypes.RinkInquiriesFormQuery;
}): JSX.Element => {
  const {
    register,
    handleSubmit,
    ...
    setError,
    formState,
  } = useForm<RinkInquiryFormValues>();
  const [showThankYou, setShowThankYou] = useState(false);
  const onSuccess = () => {
    setShowThankYou(true);
  };

  invariant(formJson, 'Rink Inquiry Form JSON data is required!');
...
export default RinkInquiryForm;
