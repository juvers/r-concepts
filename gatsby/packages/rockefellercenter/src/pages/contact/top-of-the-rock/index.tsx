...
export const query = graphql`
  query TopOfTheRockForm {
    meta: formJson(id: {eq: "top-of-the-rock-form"}) {
      meta {
        title
        description
      }
    }
    formJson(id: {eq: "top-of-the-rock-form"}) {
      id
      integrationId
      name
      slug
...
interface TopOfTheRockInquiryFormValues {
  fullname: string;
  email: string;
  phone: string;
  comments?: string;
...
const TopOfTheRockInquiryForm = ({
  data: {formJson},
}: {
  data: GatsbyTypes.TopOfTheRockFormQuery;
}): JSX.Element => {
  const {
    register,
    handleSubmit,
    ...
    setError,
    formState,
  } = useForm<TopOfTheRockInquiryFormValues>();
  const [showThankYou, setShowThankYou] = useState(false);
  const onSuccess = () => {
    setShowThankYou(true);
  };

  invariant(formJson, 'Top of the Rock Inquiry Form JSON data is required!');
...
export default TopOfTheRockInquiryForm;
