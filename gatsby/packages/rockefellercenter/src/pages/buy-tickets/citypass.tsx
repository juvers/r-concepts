...
export default function CityPassTickets({
  data,
}: {
  data: GatsbyTypes.BuyTicketsCityPassQuery;
}): JSX.Element {
  ...
  const ticketTypes = useMemo(() => {
    if (!page.ticketStep?.ticketTypes?.length)
      throw new Error('Expected ticket types for CityPass');
...
export const query = graphql`
  query BuyTicketsCityPass {
    page: buyTicketsFlowJson(id: {eq: "buy-tickets-city-pass"}) {
      hero {
        title
      }
      ticketStep {
    ...
    meta: buyTicketsFlowJson(id: {eq: "buy-tickets-city-pass"}) {
      meta {
        title
        description
      }
